...
// Add this function to your mapa.js file
function fillLocationFields(location) {
    // Assuming 'location' is an object with the address components
    document.getElementById('entidad_federativa').value = location.state || '';
    document.getElementById('municipio').value = location.municipality || '';
    document.getElementById('cp').value = location.postalCode || '';
    document.getElementById('calle').value = location.street || '';
    document.getElementById('numero_exterior').value = location.exteriorNumber || '';
    document.getElementById('numero_interior').value = location.interiorNumber || '';
    
    console.log('Location data saved to form fields:', location);
}

// Call this function when you get the location data
// For example, after a successful geocoding operation:
// geocoder.geocode({ 'location': latlng }, function(results, status) {
//     if (status === 'OK') {
//         // Process the results to extract address components
//         const location = processGeocodingResults(results);
//         fillLocationFields(location);
//     }
// });

// Example function to process geocoding results
function processGeocodingResults(results) {
    ...
    for (const component of addressComponents) {
        ...
        if (types.includes('administrative_area_level_1')) {
            ...
            location.municipality = component.long_name;
        } else if (types.includes('postal_code')) {
            location.postalCode = component.long_name;
        } else if (types.includes('route')) {
            location.street = component.long_name;
        } else if (types.includes('street_number')) {
            location.exteriorNumber = component.long_name;
        }
    }
...
function fillLocationFields(location) {
    document.getElementById('entidad_federativa').value = location.state || '';
    document.getElementById('municipio').value = location.city || '';
    document.getElementById('cp').value = location.postalCode || '';
    document.getElementById('calle').value = location.street || '';
    document.getElementById('numero_exterior').value = location.streetNumber || '';
    document.getElementById('numero_interior').value = '';
    
    console.log('Location data saved to form fields:', {
        entidad_federativa: location.state,
        municipio: location.city,
        cp: location.postalCode,
        calle: location.street,
        numero_exterior: location.streetNumber
    });
}

// Add this function to your mapa.js file
// This should be called whenever you get location data from your map

function saveLocationData(locationData) {
    // Check if the saveLocationToForm function exists in the window object
    if (typeof window.saveLocationToForm === 'function') {
        // Call the function with the location data
        window.saveLocationToForm({
            entidad_federativa: locationData.state || locationData.entidad_federativa,
            municipio: locationData.city || locationData.municipio,
            cp: locationData.postalCode || locationData.cp,
            calle: locationData.street || locationData.calle,
            numero_exterior: locationData.streetNumber || locationData.numero_exterior,
            numero_interior: locationData.interiorNumber || locationData.numero_interior
        });
    } else {
        console.error('saveLocationToForm function not found');
    }
}
